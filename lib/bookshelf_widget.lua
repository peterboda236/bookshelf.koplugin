...
function BookshelfWidget:_rebuildRefreshBelowHero()
    ...
    if hero_dimen and hero_dimen.y and hero_dimen.h then
        below_y, src = hero_dimen.y + hero_dimen.h, "live"
    elseif self._hero_dims and self._hero_dims.hero_h then
        below_y, src = (self._hero_dims.PAD or 0) + self._hero_dims.hero_h, "stashed"
    end
    logger.info(string.format("[bookshelf #124] chip/page refresh=%s below_y=%s",
        below_y and ("scoped-" .. src) or "FULL-SCREEN", tostring(below_y)))
    if below_y then
